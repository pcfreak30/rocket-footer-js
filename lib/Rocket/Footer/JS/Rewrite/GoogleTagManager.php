...
class GoogleTagManager extends RewriteAbstract {
	...
	protected function do_rewrite( $content, $src ) {
		if ( preg_match( '~\(\s*function\s*\(\s*w\s*,\s*\s*d,\s*s,\s*l,\s*i\s*\)\s*\{\s*w\s*\[\s*l\s*\]\s*=\s*w\s*\[\s*l\]\s*\|\|\s*\[\s*\]\s*;\s*w\s*\[\s*l\s*\].*\(\s*window\s*,\s*document\s*,\s*\'script\'\s*,\s*\'.*\'\s*,\s*\'([\w]+-[\w]+)\'\s*\);~s', $content, $matches ) ) {
